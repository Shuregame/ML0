...
Метод парзеновского окна использует весовую функцию w(i,u) как функцию не от ранга (как в KNN), а от расстояния. Зададим некоторый параметр h - ширина окна (напр. в двумерном пространстве это радиус круга).Бывают случаи, когда объект не лежит среди плотного распределения точек, в таком случае лучше сделать ширину с переменным значением.
